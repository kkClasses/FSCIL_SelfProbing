...
def writeFile(logits1, logits2, logits3, logits,i, test_label, proto):
    ...
    if selected_logits1:  # Ensure non-empty selection before concatenation
        ...
        # selected_test_label = test_label[divisible_by_25].unsqueeze(1)
        # mx, ind = torch.max(selected_logits1, dim=1)
        # mx1 = torch.gather(selected_logits2, dim=1, index=ind.unsqueeze(1))
        # mx_unsqueezed = mx.unsqueeze(1)  # Shape: [batch_size, 1]
        # result_tensor = torch.cat([ ind.unsqueeze(1), diff1, ind2.unsqueeze(1), diff2, selected_test_label], dim=1)
